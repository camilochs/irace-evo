...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // Heuristic: Prioritize runs that are both long and minimally overlapping with others,
    // using a precomputed "conflict score" to bias selection toward less conflicting runs.
    // This encourages selection of compatible, high-coverage runs early.

    static vector<int> run_lengths;
    static vector<int> run_conflicts;
    static int last_init = -1;

    if (last_init != n_of_runs) {
        // Precompute run lengths and conflict counts (number of overlaps with other runs)
        run_lengths.assign(n_of_runs, 0);
        run_conflicts.assign(n_of_runs, 0);

        // Build interval endpoints for sweep line
        vector<pair<int, int>> endpoints; // {pos, +run_id or -run_id}
        for (int i = 0; i < n_of_runs; ++i) {
            run_lengths[i] = runs[i].length;
            endpoints.emplace_back(runs[i].start, i + 1); // +run_id for start
            endpoints.emplace_back(runs[i].start + runs[i].length - 1, -(i + 1)); // -run_id for end
        }
        ...
        for (const auto& ep : endpoints) {
            int idx = abs(ep.second) - 1;
            if (ep.second > 0) {
                // Start of run
                run_conflicts[idx] = static_cast<int>(active.size());
                for (int other : active) {
                    run_conflicts[other] += 1;
                }
                active.insert(idx);
            } else {
                active.erase(idx);
    ...
    // Heuristic score: combine random key, normalized run length, and inverse conflict score
    // Weights: random key (0.5), run length (0.3), low conflict (0.2)
    vector<pair<int, double>> pos_val_pairs(n_of_runs);
    for (int i = 0; i < n_of_runs; ++i) {
        double norm_length = static_cast<double>(run_lengths[i]) / (sequence_length + 1);
        double norm_conflict = 1.0 - (static_cast<double>(run_conflicts[i]) / (n_of_runs + 1));
        double score = 0.5 * chromosome[i] + 0.3 * norm_length + 0.2 * norm_conflict;
        pos_val_pairs[i] = {i, score};
    }
    ...
    // Restore the original chromosome value for downstream compatibility
    for (auto& p : pos_val_pairs) {
        p.second = chromosome[p.first];
    }
