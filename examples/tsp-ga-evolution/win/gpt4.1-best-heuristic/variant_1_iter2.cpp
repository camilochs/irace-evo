...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // Find min and max run length (usually small range in practice)
    int min_len = runs[0].length, max_len = runs[0].length;
    for (const Run& r : runs) {
        if (r.length < min_len) min_len = r.length;
        if (r.length > max_len) max_len = r.length;
    }
    int bucket_count = std::min(8, max_len - min_len + 1); // up to 8 buckets

    // Bucket runs by run length (longer runs get lower bucket index for output priority)
    vector<vector<pair<int, double>>> buckets(bucket_count);
    int len_range = max_len - min_len + 1;
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        int bucket_idx = (len_range <= bucket_count)
            ? (max_len - runs[i].length)
            : ((max_len - runs[i].length) * bucket_count / len_range);
        bucket_idx = std::min(std::max(bucket_idx, 0), bucket_count-1);
        buckets[bucket_idx].emplace_back(i, chromosome[i]);
    }

    // Stable sort within each bucket by chromosome value descending (preserve input order for ties)
    ...
    // Output: interleave, prioritizing longer runs (lower bucket index first)
    vector<pair<int, double>> result;
    result.reserve(chromosome.size());
    // Greedy round-robin interleaving for diversity, longer runs first
    bool added = true;
    while (added) {
        added = false;
        for (auto& bucket : buckets) {
