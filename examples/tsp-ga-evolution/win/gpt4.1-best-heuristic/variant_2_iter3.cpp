...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY INSIGHT: Ultra-fast, atomic rank fusion: independently rank by chromosome value and by run start position, sum ranks for diversity.
    // Rationale: Explores both high-value and spatially early runs, avoids costly sorts and buckets.

    int n = static_cast<int>(chromosome.size());
    vector<int> idx(n);
    for (int i = 0; i < n; ++i) idx[i] = i;

    // Rank by chromosome value (descending)
    vector<int> value_rank(n);
    {
        vector<int> tmp(idx);
        std::sort(tmp.begin(), tmp.end(), [&](int a, int b) {
            return chromosome[a] > chromosome[b];
        });
        for (int rank = 0; rank < n; ++rank) value_rank[tmp[rank]] = rank;
    }

    // Rank by run start (ascending, i.e., earlier runs get lower rank)
    vector<int> start_rank(n);
    {
        vector<int> tmp(idx);
        std::sort(tmp.begin(), tmp.end(), [&](int a, int b) {
            return runs[a].start < runs[b].start;
        });
        for (int rank = 0; rank < n; ++rank) start_rank[tmp[rank]] = rank;
    }

    // Combine ranks: lower sum = higher priority (atomic, no tie-breaking)
    vector<pair<int, int>> fused_rank(n);
    for (int i = 0; i < n; ++i)
        fused_rank[i] = { value_rank[i] + start_rank[i], i };

    std::sort(fused_rank.begin(), fused_rank.end());

    vector<pair<int, double>> result;
    result.reserve(n);
    for (int k = 0; k < n; ++k) {
        int i = fused_rank[k].second;
        result.emplace_back(i, chromosome[i]);
    }
    return result;
}
