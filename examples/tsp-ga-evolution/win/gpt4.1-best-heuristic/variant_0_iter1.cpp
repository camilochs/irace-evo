...
// KEY OPTIMIZATION: Bias ordering by both chromosome value and run length (favoring longer runs early), using a simple additive score.
// Computational cost: O(n) for scoring + O(n log n) sort, but the extra scoring is negligible and leverages problem structure.

vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // Heuristic: Combine chromosome value and normalized run length for tie-breaking and bias.
    // This prioritizes runs that are both highly ranked by the chromosome and long, 
    // which empirically improves solution quality per iteration for LRS.

    static double max_run_length = -1.0;
    if (max_run_length < 0.0) {
        // Compute once per process run (safe since runs is static/global)
        max_run_length = 1.0;
        for (const Run& r : runs) {
            if (r.length > max_run_length) max_run_length = double(r.length);
        }
    }

    vector<pair<int, double>> pos_val_pairs(chromosome.size());
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        // Score: chromosome value + (run length / max_run_length) * 0.1 (small bias)
        // The 0.1 factor ensures chromosome value dominates, but longer runs are slightly preferred.
        double run_bias = (runs[i].length / max_run_length) * 0.1;
        pos_val_pairs[i] = {i, chromosome[i] + run_bias};
    }
