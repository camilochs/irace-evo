...
// KEY OPTIMIZATION: Fast, structure-aware bucketing by run length (favoring longer runs), 
// then stable sort within each bucket by chromosome value (descending). 
// This exploits the LRS objective (maximize run length) while maintaining high throughput (O(n) bucketization, O(n log k) for k buckets).
// Avoids spatial clustering, overlap checks, and costly global statistics.

vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: "Alternating Greedy Sweep" - alternate between selecting the max run by chromosome value
    // and the max run by run length, rapidly covering both exploitation and exploration in O(n) time.
    // No sorting, no per-run randomization, no bucketing: just two linear scans and a merge.

    int n = static_cast<int>(chromosome.size());
    vector<pair<int, double>> result;
    result.reserve(n);

    // 1. Find indices sorted by chromosome value (descending) and by run length (descending), using index arrays.
    vector<int> idx_by_chrom(n), idx_by_length(n);
    std::iota(idx_by_chrom.begin(), idx_by_chrom.end(), 0);
    std::iota(idx_by_length.begin(), idx_by_length.end(), 0);

    // Partial sorts: only produce index orderings, not full pair objects.
    std::partial_sort(idx_by_chrom.begin(), idx_by_chrom.end(), idx_by_chrom.end(),
        [&](int a, int b) { return chromosome[a] > chromosome[b]; });
    std::partial_sort(idx_by_length.begin(), idx_by_length.end(), idx_by_length.end(),
        [&](int a, int b) { return runs[a].length > runs[b].length; });

    // 2. Merge alternately from both orderings, skipping duplicates.
    vector<bool> used(n, false);
    int i = 0, j = 0;
    while (result.size() < static_cast<size_t>(n)) {
        if (i < n && !used[idx_by_chrom[i]]) {
            int idx = idx_by_chrom[i++];
            result.emplace_back(idx, chromosome[idx]);
            used[idx] = true;
        }
        if (j < n && !used[idx_by_length[j]]) {
            int idx = idx_by_length[j++];
            result.emplace_back(idx, chromosome[idx]);
            used[idx] = true;
        }
    }
