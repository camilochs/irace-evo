...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Fast selection bias by penalizing runs that overlap with the previous run's end,
    // using a simple, local, non-global penalty. This encourages non-overlapping, spread-out selections
    // with minimal computational overhead (O(n)), and avoids global normalization or run-length bias.

    vector<pair<int, double>> pos_val_pairs(chromosome.size());
    int last_end = -1;
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        // If this run starts immediately after the previous, apply a small penalty
        double penalty = 0.0;
        if (last_end >= 0 && runs[i].start <= last_end) {
            penalty = 0.05; // Small, fixed penalty for overlap/adjacency
        }
        pos_val_pairs[i] = {i, chromosome[i] - penalty};
        last_end = runs[i].start + runs[i].length - 1;
    }
