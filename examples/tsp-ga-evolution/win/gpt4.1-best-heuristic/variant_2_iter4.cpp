...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Purely atomic, ultra-fast block partitioning by run parity (even/odd start index),
    // then within each block, preserve original chromosome ordering for maximal throughput and diversity.
    // No sorting, no ranking, no hybrid scoring: just O(n) partition and linear scan.
    // This leverages spatial alternation and chromosome bias for aggressive exploration.

    int n = static_cast<int>(chromosome.size());
    vector<pair<int, double>> even_block;
    vector<pair<int, double>> odd_block;
    even_block.reserve(n / 2 + 1);
    odd_block.reserve(n / 2 + 1);

    // Partition runs by even/odd start index (atomic, O(n))
    for (int i = 0; i < n; ++i) {
        if ((runs[i].start & 1) == 0)
            even_block.emplace_back(i, chromosome[i]);
        else
            odd_block.emplace_back(i, chromosome[i]);
    }

    // Alternate block order every call to maximize exploration (static toggle, thread-safe)
    ...
    result.reserve(n);
    if (toggle) {
        result.insert(result.end(), even_block.begin(), even_block.end());
        result.insert(result.end(), odd_block.begin(), odd_block.end());
    } else {
        result.insert(result.end(), odd_block.begin(), odd_block.end());
        result.insert(result.end(), even_block.begin(), even_block.end());
    }
