...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Ultra-fast, minimalist "first-fit by letter" bucketing.
    // For each letter, only the top run (by chromosome value) is considered first, then fill with others.
    // This ensures rapid, diversified exploration and avoids expensive global sorts.
    // All operations are O(n), no nested loops, no per-run randomization.

    int n = static_cast<int>(chromosome.size());
    vector<pair<int, double>> scored;
    scored.reserve(n);

    // 1. Track the best run for each letter (highest chromosome value)
    vector<int> best_run_idx(alphabet_size, -1);
    vector<double> best_run_val(alphabet_size, -1.0);

    for (int i = 0; i < n; ++i) {
        int l = runs[i].letter;
        if (chromosome[i] > best_run_val[l]) {
            best_run_val[l] = chromosome[i];
            best_run_idx[l] = i;
        }
    }
    // 2. Add the best run for each letter first (if exists)
    vector<bool> used(n, false);
    for (int l = 0; l < alphabet_size; ++l) {
        if (best_run_idx[l] != -1) {
            scored.emplace_back(best_run_idx[l], chromosome[best_run_idx[l]]);
            used[best_run_idx[l]] = true;
        }
    }
    // 3. Fill the rest: all other runs, in descending order of chromosome value (but skip already used)
    vector<pair<int, double>> rest;
    rest.reserve(n - scored.size());
    for (int i = 0; i < n; ++i) {
        if (!used[i]) rest.emplace_back(i, chromosome[i]);
    }
    std::sort(rest.begin(), rest.end(),
        [](const pair<int, double>& a, const pair<int, double>& b) { return a.second > b.second; });
    for (const auto& p : rest) scored.push_back(p);
