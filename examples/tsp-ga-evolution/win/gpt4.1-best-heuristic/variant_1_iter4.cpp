...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Aggressive, atomic hybrid scoring: multiply run length by chromosome value (favoring both long and high-key runs), 
    // with a tiny random jitter to break ties and enhance exploration. 
    // All operations are O(n), and only a single sort is performed.

    static thread_local std::minstd_rand rng(std::random_device{}());
    static thread_local std::uniform_real_distribution<double> jitter_dist(0.0, 1e-6);

    int n = static_cast<int>(chromosome.size());
    vector<pair<int, double>> scored(n);
    for (int i = 0; i < n; ++i) {
        // Lightweight hybrid: prioritize long runs with high chromosome values, add tiny noise for diversity
        scored[i] = {i, runs[i].length * chromosome[i] + jitter_dist(rng)};
    }
