...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Fast, non-iterative spatial separation bias.
    // Assign each run a priority by combining its chromosome value with a bonus
    // based on its distance from the sequence center, promoting spread and reducing clustering.
    // This approach is O(n), branch-free, and requires no sorting by run length or overlap checks.

    const int n = (int)chromosome.size();
    vector<pair<int, double>> pos_val_pairs;
    pos_val_pairs.reserve(n);

    // Precompute sequence center for spatial bonus calculation
    const double seq_center = sequence_length * 0.5;

    // Use a fixed bonus for runs further from the center (to diversify selection)
    constexpr double spread_weight = 0.07;

    for (int i = 0; i < n; ++i) {
        // Distance from center, normalized to [0,1]
        double dist_center = std::abs((runs[i].start + runs[i].length * 0.5) - seq_center) / seq_center;
        // Add a small bonus for spatial diversity
        double bonus = spread_weight * dist_center;
        pos_val_pairs.emplace_back(i, chromosome[i] + bonus);
    }

    // Sort descending by adjusted value (chromosome + spatial bonus)
    std::sort(pos_val_pairs.begin(), pos_val_pairs.end(),
