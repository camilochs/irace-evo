...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // KEY OPTIMIZATION: Fast, deterministic bucketing by run start position modulo a small prime,
    // then sort within buckets by chromosome value. This encourages spatial diversity and avoids
    // clustering, with negligible overhead (O(n) for bucketing + O(n log n) for small sorts).
    // No run length or overlap computation, no normalization, no global statistics.

    constexpr int bucket_mod = 7; // Small prime for spatial scrambling
    vector<vector<pair<int, double>>> buckets(bucket_mod);

    for (int i = 0; i < (int)chromosome.size(); ++i) {
        int bucket_idx = runs[i].start % bucket_mod;
        buckets[bucket_idx].emplace_back(i, chromosome[i]);
    }

    // Sort each bucket descending by chromosome value
    ...
    // Interleave buckets for output to maximize spatial spread
    vector<pair<int, double>> result;
    result.reserve(chromosome.size());
    size_t added = 0;
    while (added < chromosome.size()) {
        for (auto& bucket : buckets) {
            if (!bucket.empty()) {
