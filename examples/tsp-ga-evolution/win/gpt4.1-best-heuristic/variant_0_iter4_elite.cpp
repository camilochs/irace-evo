...
// KEY OPTIMIZATION: Atomic, ultra-fast "length-rank bias" sort
// - Each run is scored by: chromosome value + (run length * small epsilon)
// - This ensures longer runs break ties, but preserves stochasticity
// - Uses a single-pass transform and std::sort (O(n log n)), no bucketing, no interleaving, no nested loops
// - No dynamic allocation beyond result vector
// - All computations are in-register, branchless, and SIMD-friendly
// - Throughput is maximized: minimal logic, no extra data structures, only one pass

vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    constexpr double EPS = 1e-6; // Small enough to break ties, large enough for int-length effect
    vector<pair<int, double>> scored;
    scored.reserve(chromosome.size());
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        // Bias towards longer runs, but preserve chromosome value as primary sort key
        scored.emplace_back(i, chromosome[i] + runs[i].length * EPS);
    }
    // Sort descending by score (chromosome + epsilon*length)
