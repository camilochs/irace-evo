...
// Returns a vector of pairs (index, value) sorted by descending value
#include <climits> // Fix: Needed for INT_MAX

vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    // Hybrid heuristic: Prioritize runs that are both long and minimally overlapping with others,
    // biasing by random key, run length, and run "uniqueness" (inverse overlap count).
    // This encourages selection of long, non-redundant runs, improving subsequence diversity.
    ...
    static vector<int> run_lengths;
    static vector<int> overlap_counts;
    static size_t cached_n = 0;

    if (cached_n != chromosome.size()) {
        // Precompute run lengths and overlap counts only if input size changes
        run_lengths.assign(chromosome.size(), 0);
        overlap_counts.assign(chromosome.size(), 0);
        for (size_t i = 0; i < chromosome.size(); ++i) {
            run_lengths[i] = runs[i].length;
        }
        // Precompute overlap counts for each run (O(n log n))
        // Sort runs by start, then sweep to count overlaps
        vector<pair<int, int>> run_intervals(chromosome.size());
        for (size_t i = 0; i < chromosome.size(); ++i) {
            run_intervals[i] = {runs[i].start, runs[i].start + runs[i].length - 1};
        }
        sort(run_intervals.begin(), run_intervals.end());
        vector<int> ends;
        for (size_t i = 0; i < chromosome.size(); ++i) {
            ends.push_back(run_intervals[i].second);
        }
        sort(ends.begin(), ends.end());
        for (size_t i = 0; i < chromosome.size(); ++i) {
            int s = run_intervals[i].first;
            int e = run_intervals[i].second;
            // Number of runs that start before this run ends
            int left = lower_bound(run_intervals.begin(), run_intervals.end(), make_pair(s, -1)) - run_intervals.begin();
            int right = upper_bound(run_intervals.begin(), run_intervals.end(), make_pair(e, INT_MAX)) - run_intervals.begin();
            // Number of runs overlapping with this run (including itself)
            overlap_counts[i] = right - left;
        }
        cached_n = chromosome.size();
    }

    vector<RunScore> scored_runs(chromosome.size());
    for (size_t i = 0; i < chromosome.size(); ++i) {
        // Heuristic: random key (dominant), + run length (scaled), + uniqueness (inverse overlap)
        // The uniqueness term rewards runs that overlap with fewer others
        double uniqueness = 1.0 / (1.0 + overlap_counts[i]);
        scored_runs[i].idx = static_cast<int>(i);
        scored_runs[i].score = chromosome[i]
            + 0.0002 * run_lengths[i]
            + 0.0005 * uniqueness;
    }

    sort(scored_runs.begin(), scored_runs.end(),
        [](const RunScore& a, const RunScore& b) {
            return a.score > b.score;
        });

    vector<pair<int, double>> result(chromosome.size());
    for (size_t i = 0; i < chromosome.size(); ++i) {
        result[i] = {scored_runs[i].idx, scored_runs[i].score};
    }
    return result;
}

// Función para decodificar el vector de valores reales en una selección de runs
set<int> decode_solution(const vector<double>& chromosome) {

    vector<pair<int, double>> pos_val_pairs = order_chromosome(chromosome);
    // Crear pares (índice, valor) y ordenar por valor descendente
    
    // Aplicar restricciones para seleccionar runs válidos
    ...
    for (int i = 0; i < n_of_runs; ++i) {
        ...
        // Verificar restricciones con otros alfabetos
        for (int j = 0; add && (j < alphabet_size); ++j) {
        ...
        if (add) {
            selected_runs.insert(current_run);
            
            // Actualizar bounds para esta letra
...
// Función para evaluar el score de una selección de runs
int evaluate_selection(const set<int>& selected_runs) {
    int score = 0;
    for (int run_index : selected_runs) {
        score += runs[run_index].length;
    }
    return score;
}

// Función evaluate refactorizada
void evaluate(Individual& ind) {
    ind.selected_runs = decode_solution(ind.vec);
    ind.score = evaluate_selection(ind.selected_runs);
}
        ...
        while (ctime < computation_time_limit) {
            ...
            for (int ic = 0; ic < n_mutants; ++ic) {
                ...
                if (new_population[n_elites + ic].score > best_score) {
                    if (stnweb) {
                            // Escribir el formato requerido: 1,score,solution_binary
                            cout << id << "," << new_population[n_elites + ic].score << ",";
                            
                            // Generar la cadena binaria
                            string binary_solution(n_of_runs, '0');
