...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Conflict-aware ordering: prioritize runs that enable maximum future selections
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        int conflicts = 0;
        int end_pos = runs[i].start + runs[i].length - 1;
        
        // Count potential conflicts with other runs (fast O(n) scan)
        for (int j = i + 1; j < (int)chromosome.size(); ++j) {
            if (runs[j].start <= end_pos && runs[j].start + runs[j].length > runs[i].start) {
                conflicts++;
            }
        }
        
        // Inverse conflict score: fewer conflicts = higher priority
        double conflict_penalty = 1.0 / (1.0 + conflicts * 0.1);
        pos_val_pairs.emplace_back(i, chromosome[i] * conflict_penalty);
    }
