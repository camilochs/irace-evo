...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    pos_val_pairs.reserve(n_of_runs);
    
    // Novel heuristic: Subsequence gap analysis with run clustering
    // Core insight: Runs that create optimal spacing patterns have higher value
    
    // Precompute cumulative start positions for gap analysis
    vector<int> cumulative_starts(n_of_runs);
    cumulative_starts[0] = runs[0].start;
    for (int i = 1; i < n_of_runs; ++i) {
        cumulative_starts[i] = cumulative_starts[i-1] + runs[i].start;
    }
    
    // Fast gap signature computation
    const int gap_threshold = sequence_length / n_of_runs;
    
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Gap quality metric: distance to ideal spacing
        int ideal_position = (i * sequence_length) / n_of_runs;
        int gap_deviation = abs(runs[i].start - ideal_position);
        double spacing_bonus = 1.0 / (1.0 + gap_deviation * 0.01);
        
        // Run clustering coefficient: favor runs that form good clusters
        double cluster_value = 0.0;
        if (i > 0) {
            int gap_prev = runs[i].start - (runs[i-1].start + runs[i-1].length);
            cluster_value += (gap_prev < gap_threshold) ? 0.2 : -0.1;
        }
        if (i < n_of_runs - 1) {
            int gap_next = runs[i+1].start - (runs[i].start + runs[i].length);
            cluster_value += (gap_next < gap_threshold) ? 0.2 : -0.1;
        }
        
        // Length-to-gap ratio optimization
        double efficiency_ratio = runs[i].length;
        if (i < n_of_runs - 1) {
            int next_gap = runs[i+1].start - runs[i].start;
            efficiency_ratio = (next_gap > 0) ? (double)runs[i].length / next_gap : runs[i].length;
        }
        
        double enhanced_value = base_value * (1.0 + spacing_bonus * 0.3) + 
                               cluster_value + 
                               efficiency_ratio * 0.05;
        
        pos_val_pairs.emplace_back(i, enhanced_value);
    }
