...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Precompute run density map - single pass O(n)
    vector<int> density_map(sequence_length, 0);
    for (int i = 0; i < n_of_runs; ++i) {
        int end_pos = runs[i].start + runs[i].length;
        for (int pos = runs[i].start; pos < end_pos; ++pos) {
            density_map[pos]++;
        }
    }
    
    // Position-based conflict scoring - exploit spatial locality
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Density penalty: runs in crowded areas get lower priority
        int mid_pos = runs[i].start + (runs[i].length >> 1);
        double density_penalty = density_map[mid_pos] * 0.05;
        
        // Length efficiency: reward length per unit position
        double efficiency = (double)runs[i].length / (runs[i].start + 1);
        double efficiency_bonus = efficiency * 0.1;
        
        // Letter frequency bias: favor less common letters
        int letter_freq = 0;
        for (int j = 0; j < n_of_runs; ++j) {
            if (runs[j].letter == runs[i].letter) letter_freq++;
        }
        double rarity_bonus = (n_of_runs - letter_freq) * 0.001;
        
        // Position modulo pattern: exploit structural patterns
        double pattern_bonus = (runs[i].start % 7 == 0) ? 0.03 : 0.0;
        
        double final_value = base_value - density_penalty + efficiency_bonus + rarity_bonus + pattern_bonus;
        
        pos_val_pairs.emplace_back(i, final_value);
    }
