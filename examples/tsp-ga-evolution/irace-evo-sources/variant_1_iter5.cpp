...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Zone-based heuristic: Exploit run boundary structure
    // Key insight: Runs at letter boundaries have higher strategic value
    
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Boundary bonus: runs at letter transitions are more valuable
        double boundary_bonus = 0.0;
        bool is_boundary = false;
        
        // Check if this run is at a letter boundary (O(1) lookups)
        if (i == 0 || i == n_of_runs - 1) {
            is_boundary = true;
        } else if (runs[i-1].letter != runs[i].letter || runs[i+1].letter != runs[i].letter) {
            is_boundary = true;
        }
        
        if (is_boundary) {
            boundary_bonus = runs[i].length * 0.15;
        }
        
        // Length amplification with saturation to avoid overflow
        double length_factor = min(runs[i].length * 0.08, 0.5);
        
        // Simple letter diversity: penalize common letters slightly
        double letter_penalty = (runs[i].letter < alphabet_size / 2) ? -0.02 : 0.0;
        
        double enhanced_value = base_value + boundary_bonus + length_factor + letter_penalty;
