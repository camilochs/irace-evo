...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Calculate run density scores (length/position bias for better subsequence ordering)
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        double density_bias = runs[i].length * chromosome[i];
        double position_factor = 1.0 - (double(runs[i].start) / sequence_length) * 0.1;
        pos_val_pairs[i].first = i;
        pos_val_pairs[i].second = density_bias * position_factor;
    }
