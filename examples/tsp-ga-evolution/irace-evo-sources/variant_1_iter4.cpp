...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Novel heuristic: Position-aware length amplification with conflict anticipation
    // Core insight: Runs that appear early/late have different strategic values
    
    // Precompute position weights (O(1) lookup)
    const double position_factor = 2.0 / n_of_runs;
    
    // Single-pass computation with position-aware length bonus
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Position bonus: early and late runs get preference
        double position_weight = 1.0 - abs(i * position_factor - 1.0);
        
        // Length amplification with diminishing returns
        double length_bonus = runs[i].length * (1.0 + position_weight * 0.3);
        
        // Letter diversity bonus: favor less common letters
        double letter_bonus = 0.1 / (1.0 + runs[i].letter * 0.1);
        
        // Conflict anticipation: penalize runs in dense regions
        double density_penalty = 0.0;
        if (i > 0 && i < n_of_runs - 1) {
            // Simple density check: same letter neighbors
            if (runs[i-1].letter == runs[i].letter || runs[i+1].letter == runs[i].letter) {
