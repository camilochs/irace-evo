...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Entropy-based ordering: maximize information content per selection
    // Uses run letter frequency to create selection diversity
    vector<int> letter_counts(alphabet_size, 0);
    for (const auto& run : runs) {
        letter_counts[run.letter]++;
    }
    
    for (int i = 0; i < (int)chromosome.size(); ++i) {
        // Information gain heuristic: rare letters get higher priority
        double letter_rarity = 1.0 / (1.0 + letter_counts[runs[i].letter]);
        
        // Length efficiency: reward longer runs exponentially
        double length_factor = runs[i].length * runs[i].length * 0.01;
        
        // Combine with chromosome value using harmonic mean for stability
        double harmonic_weight = (2.0 * chromosome[i] * letter_rarity) / 
                               (chromosome[i] + letter_rarity + 1e-9);
        
        pos_val_pairs.emplace_back(i, harmonic_weight + length_factor);
    }
