...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Cascade scoring: exploit run sequence momentum
    // Key insight: runs that build upon previous selections create better chains
    ...
    for (int i = 0; i < n_of_runs; ++i) {
        ...
        // Momentum accumulation: runs that follow natural sequence flow
        if (i > 0 && runs[i].start >= runs[i-1].start) {
            momentum = momentum * decay_factor + runs[i].length * 0.01;
        } else {
            momentum *= 0.7;  // Reset momentum on sequence breaks
        }
        
        // Binary length classification: fast power-of-2 check
        int length_class = runs[i].length & -runs[i].length;  // Isolate rightmost bit
        double length_boost = (length_class >= 4) ? 0.06 : 0.0;
        
        // Positional hash: exploit position patterns with bit operations
        int pos_hash = (runs[i].start ^ (runs[i].start >> 3)) & 7;
        double hash_bias = (pos_hash == runs[i].letter % 8) ? 0.04 : -0.01;
        
        // Run ID parity: simple alternating preference
        double id_bias = (i % 3 == 0) ? 0.025 : 0.0;
        
        double final_value = base_value + momentum + length_boost + hash_bias + id_bias;
