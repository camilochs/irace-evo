...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Single-pass precomputation of all heuristic factors
    vector<double> length_weights(n_of_runs);
    vector<double> cluster_bonuses(n_of_runs, 0.0);
    
    // Length-based exponential weighting with saturation
    for (int i = 0; i < n_of_runs; ++i) {
        length_weights[i] = 1.0 - exp(-0.5 * runs[i].length);
    }
    
    // Letter clustering bonus: reward runs that form letter groups
    vector<int> last_seen(alphabet_size, -1);
    for (int i = 0; i < n_of_runs; ++i) {
        int letter = runs[i].letter;
        if (last_seen[letter] != -1) {
            int gap = i - last_seen[letter];
            if (gap <= 3) {
                // Recent same letter = clustering bonus
                cluster_bonuses[i] = 0.3 / gap;
                cluster_bonuses[last_seen[letter]] += 0.2 / gap;
            }
        }
        last_seen[letter] = i;
    }
    
    // Momentum-based ordering: bias toward runs that extend existing patterns
    double momentum = 0.0;
    int prev_letter = -1;
    ...
    for (int i = 0; i < n_of_runs; ++i) {
        ...
        // Update momentum based on letter transitions
        if (runs[i].letter == prev_letter) {
            momentum = min(1.0, momentum + 0.4);  // Same letter = momentum boost
        } else {
            momentum = max(0.0, momentum - 0.2);  // Different letter = momentum decay
        }
        
        // Core heuristic: weighted combination with momentum amplification
        double enhanced_value = base_value * (1.0 + length_weights[i] * 0.6) + 
                               cluster_bonuses[i] + momentum * 0.3;
        
        pos_val_pairs.emplace_back(i, enhanced_value);
        prev_letter = runs[i].letter;
    }
