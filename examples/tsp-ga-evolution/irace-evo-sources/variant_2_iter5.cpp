...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Stride-based sampling: exploit run spacing patterns
    // Key insight: runs with optimal spacing create better subsequences
    
    // Precompute stride metrics (single pass)
    vector<int> spacing_scores(n_of_runs, 0);
    for (int i = 1; i < n_of_runs; ++i) {
        int gap = runs[i].start - (runs[i-1].start + runs[i-1].length);
        spacing_scores[i] = (gap > 0) ? min(gap, 10) : 0;  // Cap at 10 for stability
    }
    
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Spacing bonus: reward runs with good gaps
        double spacing_boost = spacing_scores[i] * 0.03;
        
        // Run index parity: alternate between even/odd indices
        double parity_bonus = (i & 1) ? 0.02 : -0.01;
        
        // Length quantization: discrete length categories
        int length_tier = runs[i].length >> 2;  // Divide by 4, integer division
        double tier_bonus = (length_tier > 2) ? 0.08 : 0.0;
        
        // Letter hash: simple hash-based letter scoring
        double letter_score = (runs[i].letter * 7) % 13 * 0.005;
        
        double final_value = base_value + spacing_boost + parity_bonus + tier_bonus + letter_score;
