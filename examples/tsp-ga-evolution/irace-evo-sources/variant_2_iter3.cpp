...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Single-pass computation with minimal overhead
    // Core insight: Treat runs as binary features for ultra-fast classification
    
    // Precompute run signatures as bit patterns for O(1) comparisons
    vector<uint32_t> run_signatures(n_of_runs);
    uint32_t global_pattern = 0;
    
    for (int i = 0; i < n_of_runs; ++i) {
        // Encode run properties as bit pattern
        uint32_t sig = 0;
        sig |= (runs[i].length > 2) << 0;  // Length bit
        sig |= (runs[i].letter) << 1;      // Letter bits (shifted)
        sig |= (i < n_of_runs/3) << 8;     // Early position bit
        sig |= (i > 2*n_of_runs/3) << 9;   // Late position bit
        
        run_signatures[i] = sig;
        global_pattern ^= sig;  // XOR accumulation for diversity
    }
    
    // Hash-based value transformation for non-linear mixing
    const uint32_t HASH_MULT = 0x9e3779b9;
    
    for (int i = 0; i < n_of_runs; ++i) {
        double base_value = chromosome[i];
        
        // Ultra-fast heuristic: hash-based feature mixing
        uint32_t feature_hash = run_signatures[i] * HASH_MULT;
        feature_hash ^= (feature_hash >> 16);
        
        // Length bonus via bit manipulation (faster than sqrt/exp)
        int length_bonus = (runs[i].length >> 1) & 0x7;  // Cap at 7
        
        // Pattern diversity bonus: reward runs that complement global pattern
        uint32_t diversity_score = __builtin_popcount(run_signatures[i] ^ global_pattern);
        
        // Simple additive combination (fastest possible)
        double enhanced_value = base_value + 
                               (feature_hash & 0xFF) * 0.001 +
                               length_bonus * 0.1 +
