...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    vector<pair<int, double>> pos_val_pairs;
    pos_val_pairs.reserve(chromosome.size());
    
    // Precompute run density and position advantage for O(1) lookup
    vector<double> position_weight(n_of_runs);
    vector<double> run_density(n_of_runs);
    
    // Calculate position-based strategic value: early runs have exponential advantage
    // but late runs get bonus if they're long (cleanup strategy)
    for (int i = 0; i < n_of_runs; ++i) {
        double normalized_pos = double(i) / double(n_of_runs - 1);
        position_weight[i] = exp(-2.0 * normalized_pos) + 
                           (runs[i].length > 3 ? 0.3 * normalized_pos : 0.0);
        
        // Run density: length per unit position (efficiency metric)
        run_density[i] = double(runs[i].length) / (1.0 + abs(i - n_of_runs/2));
    }
    
    // Letter transition penalty: discourage frequent letter switches
    vector<int> letter_positions(alphabet_size, -1);
    for (int i = 0; i < n_of_runs; ++i) {
        letter_positions[runs[i].letter] = i;
    }
    
    for (int i = 0; i < n_of_runs; ++i) {
        // Core heuristic: Multiplicative composition of factors
        double base_value = chromosome[i];
        
        // Length dominance with diminishing returns
        double length_factor = sqrt(runs[i].length) * 0.4;
        
        // Strategic position value
        double position_factor = position_weight[i] * 0.3;
        
        // Run efficiency (density-based selection)
        double efficiency_factor = run_density[i] * 0.2;
        
        // Letter spacing bonus: reward well-distributed letters
        double spacing_bonus = 0.0;
        int current_letter = runs[i].letter;
        for (int j = 0; j < alphabet_size; ++j) {
            if (j != current_letter && letter_positions[j] != -1) {
                int distance = abs(i - letter_positions[j]);
                spacing_bonus += 1.0 / (1.0 + distance * 0.1);
            }
        }
        spacing_bonus *= 0.1;
        
        // Multiplicative combination for non-linear interactions
        double composite_score = base_value * (1.0 + length_factor) * 
                               (1.0 + position_factor) * (1.0 + efficiency_factor) + 
                               spacing_bonus;
        
        pos_val_pairs.emplace_back(i, composite_score);
    }
    
    sort(pos_val_pairs.begin(), pos_val_pairs.end(), pos_val_compare);
    return pos_val_pairs;
}
