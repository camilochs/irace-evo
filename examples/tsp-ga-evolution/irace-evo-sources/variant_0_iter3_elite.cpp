...
vector<pair<int, double>> order_chromosome(const vector<double>& chromosome) {
    ...
    // Dynamic programming approach: compute maximum achievable score from each position
    vector<int> max_future_length(n_of_runs, 0);
    vector<int> next_compatible(n_of_runs, -1);
    
    // Build compatibility chain backwards (O(n) preprocessing)
    for (int i = n_of_runs - 1; i >= 0; --i) {
        int run_end = runs[i].start + runs[i].length - 1;
        int best_next = -1;
        int best_future = 0;
        
        // Find next compatible run with maximum future potential
        for (int j = i + 1; j < n_of_runs; ++j) {
            if (runs[j].start > run_end) {
                if (runs[j].length + max_future_length[j] > best_future) {
                    best_future = runs[j].length + max_future_length[j];
                    best_next = j;
                }
            }
        }
        
        next_compatible[i] = best_next;
        max_future_length[i] = (best_next != -1) ? best_future : 0;
    }
    
    // Cascade scoring: immediate value + future opportunity
    for (int i = 0; i < n_of_runs; ++i) {
        int immediate_value = runs[i].length;
        int future_potential = max_future_length[i];
        
        // Opportunity cost heuristic: ratio of total potential to position
        double cascade_ratio = double(immediate_value + future_potential) / double(i + 1);
        
        // Letter clustering bonus: reward consecutive same-letter runs
        double cluster_bonus = 0.0;
        if (i > 0 && runs[i].letter == runs[i-1].letter) {
            cluster_bonus = 0.2;
        }
        if (i < n_of_runs - 1 && runs[i].letter == runs[i+1].letter) {
            cluster_bonus += 0.2;
        }
        
        // Final score: chromosome value weighted by cascade potential
        double final_score = chromosome[i] + cascade_ratio * 0.3 + cluster_bonus;
        
        pos_val_pairs.emplace_back(i, final_score);
    }
