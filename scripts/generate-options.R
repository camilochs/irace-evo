...
is.null.or.na <- function(x) (is.null(x) | is.na.nowarn(x))
...
to.Rd.text <- function(x) gsub("\\\\(?:code|parameter)\\{((?:[^{}\\\\]|\\\\{|\\\\})+)\\}", "`\\1`", x, perl=TRUE)
...
for (section in ordered_sections) {
  ...
  sec.text <- apply(parameters, 1L, function(x) {
    paste0("#'      \\item{`", x["name"], "`}{",
           ifelse(is.null.or.na.or.empty(x["man"]),
                                         to.Rd.text(x["description"]), x["man"]),
                  " (Default: `",
...
gendefparameter <- function(x)
{
  sprintf(" \\defparameter%s{%s}{%s}{%s}%%\n %s\n",
          ...
          ifelse(is.null.or.na.or.empty(x["vignettes"]), x["description"], x["vignettes"]))
}
...
for (section in ordered_sections) {
  ...
  missing_vignettes <- is.null.or.na.or.empty(parameters[,"vignettes"])
  # parameters <- parameters[!is.na(parameters[,"vignettes"]), ]
  parameters[missing_vignettes, "vignettes"] <- parameters[missing_vignettes, "description"]
  parameters <- parameters[!is.null.or.na.or.empty(parameters[,"vignettes"]), , drop = FALSE]

  if (nrow(parameters) < 1L) next
  sec.text <- apply(parameters, 1L, gendefparameter)
...
for (i in seq_len(nrow(irace_params))) {
  ...
  if (is.null.or.na(p["description"]))
    stop("Missing description for option:", p["name"], "!")
...
paste0(trimws(deparse(irace_params), which="right"), collapse = "\n"),
